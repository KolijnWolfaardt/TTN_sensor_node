...
char RADIO_SLEEP_STRING[] = "sys sleep 15000\r\n";
...
int main(void)
{
    ...
    {  
        ...
        else if (CURRENT_STATE == GET_MAC_STATUS)
        {
            ...
            response_length = send_and_wait(MAC_STATUS_STRING, MAC_STATUS_STR_LEN, 40);   
        ...
        {
            response_length = send_and_wait(MAC_JOIN_STRING, MAC_JOIN_STR_LEN, 100);

            if (response_length == -1)
            {
                CURRENT_STATE = HANDLE_ISSUE;
            }
            else if (message_matches(RESPONSE_OK, RESPONSE_OK_LEN))
            {
        ...
        {
            send_and_wait(MAC_CONFIG_STRING, MAC_CONFIG_STR_LEN, 50);
        ...
        else if (CURRENT_STATE == SEND_VALUES)
        {
            ...
            response_length = send_and_wait(TRANSMIT_STRING, TRANSMIT_STR_LEN, 100);

            if (message_matches(RESPONSE_OK, RESPONSE_OK_LEN)) 
            {
                //CURRENT_STATE = SEND_VALUES_RESPONSE;
        ...
        {
            clear_message();
            uart1_send_message(RADIO_SLEEP_STRING, RADIO_SLEEP_STR_LEN);
            CURRENT_STATE = SLEEP;
            LED3 = 0;
        }
...
void reset_radio()
{
    ...
    uart1_send_message(temp_buffer,1);
...
//    BAT_SENS_DISABLE = 0;
    ...
    //Wait for the values to stabalize
    for (int i = 0; i < 100 ; i++ );
...
//    BAT_SENS_DISABLE = 1;
    ...
    // This is hacky
    TRANSMIT_STRING[15] = HEX_LOOKUP[(battery_voltage>>4) & 0x0F];
    TRANSMIT_STRING[16] = HEX_LOOKUP[battery_voltage & 0x0F];
    TRANSMIT_STRING[17] = HEX_LOOKUP[(ldr_value>>4) & 0x0F];
    TRANSMIT_STRING[18] = HEX_LOOKUP[ldr_value & 0x0F];
    TRANSMIT_STRING[19] = HEX_LOOKUP[(temp_sens>>4) & 0x0F];
    TRANSMIT_STRING[20] = HEX_LOOKUP[temp_sens & 0x0F];
    TRANSMIT_STRING[21] = HEX_LOOKUP[(light_sensor>>4) & 0x0F];
    TRANSMIT_STRING[22] = HEX_LOOKUP[light_sensor & 0x0F];
}
